...
// emptyJSONArray is the canonical empty jsonb array literal used for
// contributing_sources when an edge has no recorded contributors yet.
var emptyJSONArray = []byte("[]")

// marshalContributingSources marshals an EdgeContributingSource slice to a
// jsonb-compatible byte slice. When the slice is empty, the edge's current
// Source is used as the single seed entry so dedup-time merges always see at
// least one contributor (matches the in-memory invariant produced by
// DeduplicateEdgesWithPriority).
func marshalContributingSources(srcs []EdgeContributingSource, fallbackSource string) ([]byte, error) {
	if len(srcs) > 0 {
	...
	return json.Marshal([]EdgeContributingSource{
		{Source: fallbackSource, LastSeenAt: time.Now()},
	})
}
...
type Service struct {
	...
	// podSynth synthesizes Pod entities + Pod→Node RUNS_ON and
	// Workload→Pod MANAGES edges on read. Pods are never persisted;
	// see pod_synthesizer.go.
	podSynth *PodSynthesizer
}
...
const (
	...
	FlowSourceCategoryManual     FlowSourceCategory = "manual"
)
...
const (
	EdgeBatchSize = 4500 // × len(edgeUpsertCols)=14 → 63,000 params
...
func NewService(ctx *security.RequestContext, logger *slog.Logger, dbManager *database.DatabaseManager) *Service {
	...
	return &Service{
		...
		podSynth: NewPodSynthesizer(dbManager, logger),
	}
}
...
// alwaysOnFlowSources lists flow sources that must run on every build cycle
// regardless of the tenant filter. These are flow sources whose work is
// gated by the *presence of tenant-owned rows* (e.g. the `manual` source
// reads from kg_manual_dependencies), so the act of populating the source's
// backing data IS the opt-in — operators shouldn't have to also toggle a
// flag in the tenant filter and wonder why their declared edges never
// appeared. The flow source itself is a no-op when its backing table holds
// zero rows for the tenant, so the overhead is one bounded SELECT per
// tenant per build cycle. Keep this list alphabetically sorted.
var alwaysOnFlowSources = []string{"manual"}

// applyAlwaysOnFlowSources returns `requested` augmented with every entry in
// `alwaysOnFlowSources` that wasn't already present. Order is preserved and
// duplicates are skipped. Called at every entry point that materializes
// req.FlowSources so the always-on guarantee holds regardless of how the
// caller assembled the list (cron / queue / direct RPC).
func applyAlwaysOnFlowSources(requested []string) []string {
	seen := make(map[string]bool, len(requested)+len(alwaysOnFlowSources))
...
func (s *Service) getApplicableSourcesForAccount(cloudProvider string, requestedSources []string) ([]string, []string) {
	...
	sourceMapping := map[string][]string{
		...
		"Azure": {"azure"}, // Azure accounts can use Azure sources
...
func (s *Service) BuildGraphs(ctx *security.RequestContext, req *BuildRequest) (*BuildResponse, error) {
	...
	// Always-on flow sources (e.g. `manual`) get appended regardless of what
	// the tenant filter said. This is the canonical injection point — every
	// downstream check below (the Phase 2.5 gate, buildFlowRelationships, the
	// "did flow sources successfully run?" tracking) sees the final list.
	req.FlowSources = applyAlwaysOnFlowSources(req.FlowSources)
...
func (s *Service) buildFlowRelationships(
	...
	// stay consistent). Then apply always-on injection so callers that bypass
	// BuildGraphs (direct buildFlowRelationships invocations) still get the
	// always-on guarantee.
	requestedFlowSources := req.FlowSources
	if len(requestedFlowSources) == 0 {
		requestedFlowSources = s.defaultEnabledFlowSources()
	}
	requestedFlowSources = applyAlwaysOnFlowSources(requestedFlowSources)
...
func (s *Service) SaveEdges(edges []*DbEdge, nodes []*DbNode, syncVersion int64) error {
	...
	if s.dbManager == nil {
		return errors.New("knowledge graph: db manager is not configured")
	}
	...
	for i, edge := range validEdges {
		...
		// Set timestamps if not present; normalize any upstream-supplied
		// CreatedAt to UTC so a local-time value from a source doesn't get
		// stored as local wall-clock in the no-tz timestamp column.
		...
		// Marshal contributing_sources (jsonb). Empty array when nil so dedup-time
		// merges always see an array shape rather than NULL.
		contribJSON, err := marshalContributingSources(edge.ContributingSources, edge.Source)
		if err != nil {
			return fmt.Errorf("failed to marshal edge contributing_sources: %w", err)
		}
	...
	onConflictUpdate := []string{"updated_at", "properties", "is_active", "last_sync_version", "source", "contributing_sources"}

	// `contributing_sources` must UNION the existing column with the incoming
	// row, not overwrite. Without the override, `col = excluded.col` replaces
	// `[{k8s},{traces}]` with `[{manual}]` on every dedup rewrite — silently
	// erasing the corroboration history that downstream code (correlation
	// weighting, UI diff view) relies on.
	//
	// Merge rule (per-source dedupe, keep the latest last_seen_at): unnest
	// both arrays, group by `source`, take MAX(last_seen_at), reassemble.
	// Fallback to `excluded.contributing_sources` for the rare case the merge
	// produces an empty array (e.g., both sides empty or all entries missing
	// a `source` key) so we never write NULL.
	conflictUpdateExpr := map[string]string{
		"contributing_sources": `COALESCE((
			SELECT jsonb_agg(jsonb_build_object('source', source_name, 'last_seen_at', max_seen))
			FROM (
				SELECT
					elem->>'source' AS source_name,
					MAX(elem->>'last_seen_at') AS max_seen
				FROM jsonb_array_elements(
					COALESCE(knowledge_graph_edge.contributing_sources, '[]'::jsonb)
					|| COALESCE(excluded.contributing_sources, '[]'::jsonb)
				) elem
				WHERE elem->>'source' IS NOT NULL
				GROUP BY elem->>'source'
			) merged
		), excluded.contributing_sources)`,
	}
	...
	for i := 0; i < len(values); i += EdgeBatchSize {
		...
		batch := values[i:end]
		_, err := s.dbManager.InsertWithUpsertExpressions(tx, "knowledge_graph_edge", onConflict, onConflictUpdate, conflictUpdateExpr, nil, cols, batch...)
		if err != nil {
...
func (s *Service) GetNodeNeighbors(reqCtx *security.RequestContext, nodeID string) (KnowledgeGraph, error) {
	...
	// Synth-Pod-as-seed: synth Pods are not in knowledge_graph_node,
	// so the SQL path below would return "node not found". Route here
	// instead. NeighborsForPod returns the Pod + its parent Node and
	// (where the parent kind is supported) its parent Workload, plus
	// the corresponding RUNS_ON / MANAGES edges.
	if s.podSynth != nil {
		tenantID := ""
		if reqCtx != nil {
			tenantID = reqCtx.GetSecurityContext().GetTenantId()
		}
		if tenantID != "" && s.podSynth.IsPodSeed(tenantID, nodeID) {
			return s.getSynthPodNeighbors(tenantID, nodeID)
		}
	}
	...
	// Step 5: Synth fan-out. Pod entities are not persisted; for K8s
	// Node and Workload targets we read public.k8s_pods directly to
	// give callers the same Pod-on-Node / Pod-in-Workload view the
	// UI Pods tab serves from k8s_pods. See pod_synthesizer.go.
	if s.podSynth != nil {
		switch targetNode.NodeType {
		case NodeTypeNode:
			if pods, podEdges, fanErr := s.podSynth.PodsForNode(targetNode, 0); fanErr != nil {
				s.logger.Warn("pod synth fan-out (Node) failed", "node_id", nodeID, "error", fanErr)
			} else {
				allNodes = append(allNodes, pods...)
				edges = append(edges, podEdges...)
			}
		case NodeTypeWorkload:
			if pods, podEdges, fanErr := s.podSynth.PodsForWorkload(targetNode, 0); fanErr != nil {
				s.logger.Warn("pod synth fan-out (Workload) failed", "node_id", nodeID, "error", fanErr)
			} else {
				allNodes = append(allNodes, pods...)
				edges = append(edges, podEdges...)
			}
...
// includesPod reports whether a node-type filter slice should pass Pod
// entities through. Empty filter ⇒ no restriction ⇒ Pods allowed.
func includesPod(nodeTypes []NodeType) bool {
	if len(nodeTypes) == 0 {
		return true
	}
...
func (s *Service) fanOutPodsForNodesAndWorkloads(allNodes []*DbNode) ([]*DbNode, []*DbEdge) {
	...
	var synthEdges []*DbEdge
	seenPodIDs := make(map[string]struct{})
	seenEdgeIDs := make(map[string]struct{})
	appendPods := func(pods []*DbNode, edges []*DbEdge) {
		for _, p := range pods {
			if _, ok := seenPodIDs[p.ID]; ok {
				continue
			}
			seenPodIDs[p.ID] = struct{}{}
			synthPods = append(synthPods, p)
		}
		for _, e := range edges {
			if _, ok := seenEdgeIDs[e.ID]; ok {
				continue
			}
			seenEdgeIDs[e.ID] = struct{}{}
			synthEdges = append(synthEdges, e)
		}
	}
	for _, n := range allNodes {
		...
		case NodeTypeNode:
			...
			if err != nil {
				s.logger.Warn("pod synth fan-out (Node) failed", "node_id", n.ID, "error", err)
				continue
			}
			appendPods(pods, podEdges)
		case NodeTypeWorkload:
			pods, podEdges, err := s.podSynth.PodsForWorkload(n, 0)
			if err != nil {
				s.logger.Warn("pod synth fan-out (Workload) failed", "node_id", n.ID, "error", err)
				continue
			}
			appendPods(pods, podEdges)
		}
	}
	return synthPods, synthEdges
}

// getSynthPodNeighbors handles the branch of GetNodeNeighbors where the
// requested nodeID is a synth Pod (resolved via cloud_resource_id in
// public.k8s_pods). It returns the Pod itself, its parent Node, and —
// for supported parent kinds — its parent Workload, plus the
// corresponding RUNS_ON / MANAGES edges.
func (s *Service) getSynthPodNeighbors(tenantID, nodeID string) (KnowledgeGraph, error) {
	pod, neighbors, edges, err := s.podSynth.NeighborsForPod(tenantID, nodeID)
	if err != nil {
		return KnowledgeGraph{}, fmt.Errorf("pod synth neighbors: %w", err)
	}
	if pod == nil {
		return KnowledgeGraph{}, fmt.Errorf("node not found with id: %s", nodeID)
	}

	allNodes := append([]*DbNode{pod}, neighbors...)

	accountMappings, err := s.getAccountMappings(pod.TenantID)
	if err != nil {
		s.logger.Warn(msgAccountMappingsFallback, "error", err)
		accountMappings = make(map[string]string)
	}
...
func (s *Service) GetMultipleNodeNeighbors(reqCtx *security.RequestContext, nodeIDs []string, levels int, nodeTypes []NodeType, subgraph bool) (KnowledgeGraph, error) {
	...
	// Synth-Pod-as-seed: synth Pods are not in knowledge_graph_node or
	// knowledge_graph_edge, so the CTE can't walk from them. Split the
	// input seeds into (real KG IDs, synth Pod IDs). For each synth
	// Pod, resolve its parent Node + parent Workload upfront, and feed
	// the parents to the CTE as additional seeds — that way the
	// persisted graph around each Pod's parents still expands at full
	// depth. The Pod itself + its Pod→Node / Workload→Pod edges are
	// merged into the result at the end, deduped by ID.
	var realSeeds []string
	var seedSynthPods []*DbNode
	var seedSynthEdges []*DbEdge
	if s.podSynth != nil {
		for _, id := range nodeIDs {
			if !s.podSynth.IsPodSeed(tenantID, id) {
				realSeeds = append(realSeeds, id)
				continue
			}
			pod, parents, synthEdges, synthErr := s.podSynth.NeighborsForPod(tenantID, id)
			if synthErr != nil {
				s.logger.Warn("pod synth seed resolution failed", "node_id", id, "error", synthErr)
				continue
			}
			if pod == nil {
				continue
			}
			seedSynthPods = append(seedSynthPods, pod)
			seedSynthEdges = append(seedSynthEdges, synthEdges...)
			for _, parent := range parents {
				realSeeds = append(realSeeds, parent.ID)
			}
		}
	} else {
		realSeeds = nodeIDs
	...
	s.logger.Info("retrieving multiple nodes and their neighbors from database",
		...
		"real_seed_count", len(realSeeds),
		"synth_seed_count", len(seedSynthPods),
		"levels", levels,
		"node_types_filter", nodeTypes,
		"subgraph", subgraph)

	// If every input seed was a synth Pod with no persisted parent, the
	// CTE has nothing to walk — return just the synth Pods + their
	// edges. Without this short-circuit the CTE would error on an
	// empty seed array.
	if len(realSeeds) == 0 {
		if len(seedSynthPods) == 0 {
			return KnowledgeGraph{}, fmt.Errorf("no nodes found with provided ids")
		}
		accountMappings, mapErr := s.getAccountMappings(tenantID)
		if mapErr != nil {
			s.logger.Warn(msgAccountMappingsFallback, "error", mapErr)
			accountMappings = make(map[string]string)
		}
		return KnowledgeGraph{
			Nodes:       ConvertDbNodesToKgNodesWithAccountNames(seedSynthPods, accountMappings),
			Edges:       ConvertDbEdgesToKgEdges(seedSynthEdges),
			TenantID:    tenantID,
			AccountID:   seedSynthPods[0].CloudAccountID,
			GeneratedAt: time.Now(),
		}, nil
	}
	...
	if len(discoveredNodeIDs) == 0 && len(seedSynthPods) == 0 {
		return KnowledgeGraph{}, fmt.Errorf("no nodes found with provided ids")
	}
	...
	if len(allNodes) == 0 && len(seedSynthPods) == 0 {
		return KnowledgeGraph{}, fmt.Errorf("no nodes found with provided ids")
	}
	...
	// Synth Pod fan-out. Pod entities are not in knowledge_graph_node, so
	// the recursive CTE cannot reach them — they are leaves. For every
	// discovered Node / Workload, read public.k8s_pods and append Pod
	...
	// Merge synth-Pod-as-seed results. The synth Pod itself never
	// appears in CTE/fanOut output for its own seed lookup (the CTE
	// starts from the parent, fanOut emits the seed Pod alongside its
	// siblings, but only if includesPod(nodeTypes) is true and the
	// parent Node/Workload was reached). Dedup by ID so the seed Pod
	// shows up exactly once regardless of which path discovered it.
	if len(seedSynthPods) > 0 {
		seenNodes := make(map[string]struct{}, len(allNodes))
		for _, n := range allNodes {
			seenNodes[n.ID] = struct{}{}
		}
		for _, pod := range seedSynthPods {
			if _, ok := seenNodes[pod.ID]; !ok {
				allNodes = append(allNodes, pod)
				seenNodes[pod.ID] = struct{}{}
			}
		}
...
func (s *Service) GetNodeByID(nodeID string) (*KgNode, error) {
	...
	if len(nodes) == 0 {
		// Fall through to synthesizer: synth Pod IDs are not in
		// knowledge_graph_node. Scope to the caller's tenant for the
		// same reason GetMultipleNodeNeighbors does — globally-unique
		// UUIDs don't imply authorisation.
		if s.podSynth != nil && s.ctx != nil {
			tenantID := s.ctx.GetSecurityContext().GetTenantId()
			if tenantID != "" {
				pod, synthErr := s.podSynth.PodByID(tenantID, nodeID)
				if synthErr != nil {
					return nil, synthErr
				}
				if pod != nil {
					accountMappings, mapErr := s.getAccountMappings(pod.TenantID)
					if mapErr != nil {
						s.logger.Warn("failed to get account mappings", "error", mapErr)
						accountMappings = make(map[string]string)
					}
					kgNodes := ConvertDbNodesToKgNodesWithAccountNames([]*DbNode{pod}, accountMappings)
...
func (s *Service) SearchNodes(tenantID string, params SearchNodesParams) (*SearchNodesResponse, error) {
	...
	// permits Pods (empty or contains Pod), and they didn't restrict
	// source to a non-k8s value, ask the synthesizer to query
	// public.k8s_pods directly and merge results, respecting Limit.
	if s.podSynth != nil && includesPod(params.NodeTypes) && (params.Source == "" || params.Source == "k8s") {
		remaining := limit - len(results)
		if remaining > 0 {
			podResults, podErr := s.podSynth.SearchPods(tenantID, params, remaining)
			if podErr != nil {
				s.logger.Warn("pod synth search failed", "error", podErr)
			} else {
...
func (s *Service) TraverseDirectional(tenantID string, params TraverseParams) (*TraverseResponse, error) {
	...
	// Capture the full discovered set before any post-filter — the synth
	// Pod fan-out below iterates over Node/Workload entries here. The
	// post-filter would wipe them when the caller restricts to ["Pod"],
	// and the fan-out (which only emits Pods) would then run on an empty
	// list and return nothing.
	preFilterNodes := dbNodes
	...
	// Synth Pod fan-out. Pods aren't reachable via the persisted-edge
	// CTE, so they are leaves: for every Node / Workload in the
	// *pre-filter* discovered set we read public.k8s_pods directly.
	// Using preFilterNodes (not dbNodes) is critical when the caller
	// restricts to ["Pod"] — the post-filter wipes the parents, but
	// we still want to fan out from them. Honour MaxNodes — if the
	...
	if s.podSynth != nil && includesPod(params.NodeTypes) {
		...
		if len(synthPods) > 0 {
			...
			if len(synthPods) > remaining {
				truncated = true
				totalDiscovered += len(synthPods)
				keepIDs := make(map[string]struct{}, remaining)
				for _, p := range synthPods[:remaining] {
					keepIDs[p.ID] = struct{}{}
				}
				kept := synthEdges[:0]
				for _, e := range synthEdges {
					if _, ok := keepIDs[e.SourceNodeID]; ok {
						kept = append(kept, e)
						continue
					}
					if _, ok := keepIDs[e.DestinationNodeID]; ok {
						kept = append(kept, e)
					}
				}
				synthPods = synthPods[:remaining]
				synthEdges = kept
			} else {
				totalDiscovered += len(synthPods)
			}
			dbNodes = append(dbNodes, synthPods...)
			dbEdges = append(dbEdges, synthEdges...)
		}
	}

...
// for the resolved named constants.
//   ci.<domain>/git.repo
//   ci.<domain>/git.hash
//   ci.<domain>/git.branch
//   ci.<domain>/helm.values.filePath
//   ci.<domain>/helm.values.rootPath
//   ci.<domain>/helm.values.memoryRequestJsonPath
//   ci.<domain>/helm.values.memoryLimitJsonPath
//   ci.<domain>/helm.values.cpuRequestJsonPath
//   ci.<domain>/helm.values.cpuLimitJsonPath
//   ci.<domain>/helm.values.replicaJsonPath
//   ci.<domain>/helm.values.pvcValueJsonPath
...
func updateCode(ctx AccountAdapterContext, dir string, request ApplyRecommendationRequest, gitDetails gitDetailFromDeployment) error {
	...
	filepath, err := safeFilePath(dir, gitDetails.FilePath)
	if err != nil {
		return err
	}
	...
	rootPath := ""
	if gitDetails.Annotations[annotations.CIHelmRootPath] != "" {
	...
	case request.Recommendation.Category == "RightSizing" && request.Recommendation.RuleName == "pod_right_sizing":
		...
		// override key (`ci.<domain>/<container>.helm.values.<x>JsonPath`).
		type rightSizingResource struct {
			Category   string // "cpu" | "memory"
			Field      string // "request" | "limit"
			DefaultKey string // workload-wide annotation
			KeySuffix  string // suffix used to build the per-container override key
		}
		resources := []rightSizingResource{
			{"cpu", "request", annotations.CIHelmCPURequestJSONPath, "helm.values.cpuRequestJsonPath"},
			{"cpu", "limit", annotations.CIHelmCPULimitJSONPath, "helm.values.cpuLimitJsonPath"},
			{"memory", "request", annotations.CIHelmMemoryRequestJSONPath, "helm.values.memoryRequestJsonPath"},
			{"memory", "limit", annotations.CIHelmMemoryLimitJSONPath, "helm.values.memoryLimitJsonPath"},
		}
		for key, value := range request.Data {
			...
			for _, r := range resources {
				...
				resourceDetailsInterface, exists := resourceData[r.Category]
				if !exists {
				...
				path := fmt.Sprintf("resources.%s.%s",
					map[string]string{"request": "requests", "limit": "limits"}[r.Field],
					r.Category)
				annotationPath := ""
				if annotation, exists := gitDetails.Annotations[r.DefaultKey]; exists && annotation != "" {
					annotationPath = annotation
				}
				if annotation, exists := gitDetails.Annotations[annotations.CIKey(key+"."+r.KeySuffix)]; exists && annotation != "" {
					annotationPath = annotation
				}
				if annotationPath != "" {
					path = annotationPath
				...
				if _, exists := resourceDetails[r.Field]; !exists {
					continue // Skip if the resource field is not present
				}
				yamlPathsToUpdate[path] = resourceDetails[r.Field]
			}
		}
	...
	case request.Recommendation.Category == "RightSizing" && request.Recommendation.RuleName == "replica_right_sizing":
		if request.Data["replica_count"] != nil {
			...
			path := "replicaCount"
			if gitDetails.Annotations[annotations.CIHelmReplicaJSONPath] != "" {
				path = gitDetails.Annotations[annotations.CIHelmReplicaJSONPath]
			}
			if rootPath != "" {
				path = rootPath + "." + path
			}
...
func getGitDetailsFromRecommendation(ctx AccountAdapterContext, request ApplyRecommendationRequest) (gitDetailFromDeployment, error) {
	...
	githubRepo := workloadAnnotationsMap[annotations.CIGitRepo]
	...
	// Supports both ci.<domain>/* (for PR creation) and workloads.<domain>/* (for workload tracking)
	if githubRepo == "" {
		...
		if err == nil && workloadResourceId != "" {
			// Fetch attributes from cloud_resource_attributes (both ci.<domain> and workloads.<domain>)
			...
			if err == nil {
				for _, attr := range attributes {
					...
					case annotations.WorkloadGitRepo:
						if githubRepo == "" { // Only use if CI annotation not set
							githubRepo = attr.Value
						}
					}
				}
...
func getGitDetailsFromSecurityRecommendation(ctx AccountAdapterContext, request ApplyRecommendationRequest) (gitDetailFromDeployment, error) {
	...
	// 1. workloads.<domain>/git.repo (source code repo - preferred for security fixes)
	// 2. ci.<domain>/git.repo (CI repo - fallback)
	// 3. ArgoCD detection
	// 4. cloud_resource_attributes

	// Strategy 1: Try workloads.<domain> annotations first (source code repo)
	githubRepo := workloadAnnotationsMap[annotations.WorkloadGitRepo]
	orgBranch := workloadAnnotationsMap[annotations.WorkloadGitBranch]
	filepath := "" // For security, we let the code agent find the Dockerfile

	// Strategy 2: Fall back to ci.<domain> if no workloads annotation
	if githubRepo == "" {
		githubRepo = workloadAnnotationsMap[annotations.CIGitRepo]
		orgBranch = workloadAnnotationsMap[annotations.CIGitBranch]
	}
	...
	if githubRepo == "" && workloadResourceId != "" {
		...
		if err == nil {
			// First pass: look for workloads.<domain> (source code repo - preferred)
			for _, attr := range attributes {
				switch attr.Name {
				case annotations.WorkloadGitRepo:
					githubRepo = attr.Value
				case annotations.WorkloadGitBranch:
					orgBranch = attr.Value
				}
			}
			// Second pass: fall back to ci.<domain> if workloads not found
			if githubRepo == "" {
				for _, attr := range attributes {
					switch attr.Name {
					case annotations.CIGitRepo:
	...
	// If still no repo found, return error
	if githubRepo == "" {
		return gitDetailFromDeployment{}, common.ErrorNotFound(fmt.Sprintf("error: github repo not found in workload annotations (%s or %s), ArgoCD configuration, or cloud_resource_attributes for security recommendation", annotations.WorkloadGitRepo, annotations.CIGitRepo))
	}

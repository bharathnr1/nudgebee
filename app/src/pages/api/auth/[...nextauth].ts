...
if (process.env.TELEPORT_ENABLED == 'true') {
  // Verify Teleport JWT signatures against the cluster's JWKS endpoint.
  // If TELEPORT_JWKS_URL is missing/malformed, skip provider registration
  // entirely — accepting unsigned tokens would allow any caller to forge an
  // assertion and sign in as any user. The pod still boots so non-Teleport
  // auth methods keep working; ops will see the error log and users will
  // see the Teleport button disappear from the signin page.
  const teleportJwksUrl = process.env.TELEPORT_JWKS_URL;
  let teleportJwks: ReturnType<typeof createRemoteJWKSet> | null = null;
  if (!teleportJwksUrl) {
    console.error(
      '[Teleport] TELEPORT_ENABLED=true but TELEPORT_JWKS_URL is unset — Teleport SSO provider will NOT be registered. ' +
        "Set TELEPORT_JWKS_URL to the Teleport proxy's JWKS endpoint, e.g. https://<teleport-proxy>/.well-known/jwks.json"
    );
  } else {
    try {
      teleportJwks = createRemoteJWKSet(new URL(teleportJwksUrl));
    } catch (err) {
      console.error(
        '[Teleport] failed to initialize JWKS set — Teleport SSO provider will NOT be registered. ' +
          'Check that TELEPORT_JWKS_URL is a valid URL pointing at the Teleport proxy JWKS endpoint.',
        { teleportJwksUrl, err }
      );
      teleportJwks = null;
    }
  ...
  if (teleportJwks) {
    const verifiedJwks = teleportJwks;
    const teleportIssuer = process.env.TELEPORT_ISSUER || undefined;
    const teleportAudience = process.env.TELEPORT_AUDIENCE || undefined;
    ...
    providers.push(
      Credentials({
        ...
        async authorize(_credentials, _req) {
          ...
          if (!teleportJwt || typeof teleportJwt !== 'string') {
          ...
          try {
            ...
            if (teleportIssuer) verifyOpts.issuer = teleportIssuer;
            if (teleportAudience) verifyOpts.audience = teleportAudience;
            const verified = await jwtVerify(teleportJwt, verifiedJwks, verifyOpts);
            jwtPayload = verified.payload;
          } catch (err) {
            console.error('teleport jwt verification failed', { err });
            throw Error('Unable to verify teleport jwt assertion');
          }
    ...
    async session({ session, token }) {
      ...
      try {
        ...
        if (token) {
          ...
          nudgeBeeSession.tenant = { name: (token.tenant as any)?.name, id: (token.tenant as any)?.id };
        ...
        // EE bundles attach their session-only fields via this hook.
        // No-op in OSS extractions where the EE init-server is stripped.
        await enrichSession(nudgeBeeSession as unknown as Record<string, unknown>, token as unknown as Record<string, unknown>);
    ...
    async redirect({ url, baseUrl }) {
      ...
      try {
        ...
        if (parsedUrl.searchParams.has('callbackUrl')) {
          ...
          // Reject fully-qualified callbackUrls whose origin does not match
          // baseUrl — otherwise an attacker can craft a signin URL that
          // bounces the victim to an external host after authentication
          // (phishing or, depending on OAuth response_mode, leak of code
          // / id_token to the attacker host).
          try {
